...
const ofertaBaseSchema = z.object({
  ...
  creationYear: z.preprocess(
    emptyToUndefined,
    z.coerce.number().min(1800, 'El ano de creacion no es valido').max(new Date().getFullYear(), 'El ano no puede estar en el futuro').optional()
  ),
  ...
  extendedDescription: z.preprocess(emptyToUndefined, z.string().trim().min(1, 'La descripcion extendida es obligatoria').max(10000).optional()),
